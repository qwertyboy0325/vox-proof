...
The request binds one confirmed `InputAuthorization`, six lifecycle `RunEnvelope` stages sharing one immutable non-lifecycle posture, a sealed blind-eligible complete human-reference stack, one frozen `DetectorAnalysisIdentity`, and the exact nine-role artifact inventory (`InputAuthorization`, `ReferenceSeal`, `HumanFinalReference`, `CueReviewCompletion`, `DetectorOutput`, `EvaluationJoin`, `JoinAdjudication`, `MetricContributions`, `Metrics`). The request-level artifact inventory is directly bound to the immutable inventory carried by all six lifecycle envelopes; both surfaces must equal the canonical ordered inventory. Synchronized non-canonical envelope inventory tampering is rejected; duplicate roles receive a structured duplicate-role error. Lifecycle transitions consume the validated common envelope calibration mode; the accepted mode remains `BlindReference`. Runner request v1 accepts only `BlindReference` calibration and `SelfOwnedReal` or `ExplicitPermissionReal` input classes with `qualifies_as_real_material_evidence == true`.
...
- public completed-result replay revalidates the frozen detector snapshot against the `DetectorExecution` envelope and requires snapshot analysis identity to exactly equal the accepted validated plan; hash-synchronized substitution of another structurally valid analysis identity is rejected;
...
**Status:** The authority/export contract and implementation are owner-accepted at `3dbcb235eff601112ba9af958dd58c217429eabd`. The base Application Service implementation was previously owner-accepted at `aa26a29c11e173b63ef8c4edd6dfa4fe3f89fb59`. The bounded Gate 2 Manual Replacement implementation under MD-017 is owner-accepted at `6e395bdf9cc0a5b4333876f165280e4cc4bf506f`. This does not establish authentication, durable sessions, product persistence, HumanRaised cases, Correction Memory, external validation, or production readiness.
...
`begin_application_review` requires four inputs: `Transcript`, ordered `SessionTermEntry` values, `ApplicationMaterialUseDeclaration`, and `DeclaredSessionAuthority`. There is no unattributed three-argument constructor. `DeclaredSessionAuthority` is caller-declared, unauthenticated, not legal authorization, single-session, single-process, and non-durable. Validation rejects any Unicode control character in the original display label before trimming, then rejects an empty trimmed label. The session binds authority internally to the source revision and canonical `AnalysisSnapshot` minted at session start. Every projected decision record inherits that immutable session authority.
...
`materialize_review_export_bundle` is the only authority-complete final-export operation. It is coverage-gated like reviewed output, does not require resolved status, delegates reviewed-text and coverage semantics to the same reviewed-output path, and returns `ApplicationReviewExportBundle` containing reviewed SRT, progress, decision summary, append-ordered `ApplicationDecisionProjectionRecord` values (each inheriting session authority), declared authority, material-use basis, source revision, analysis snapshot, a path-free `ApplicationSessionSummaryProjection` with separate `ApplicationSessionOutcomeCounts`, and `ApplicationExportPosture::DeclaredOperatorUnauthenticatedInMemoryV0_2`. Bundle-only renderers `render_application_decision_log` and `render_application_session_summary` live in `application_export` and accept only the export bundle, not `ApplicationReviewSession`, `ReviewLedger`, or `CanonicalTermReviewRun`. `ReviewLedger` and `CanonicalTermReviewRun` are not public session accessors.
...
Replay verification reruns canonical analysis from the same session-owned typed source and ordered terms by immutable reference, revalidates each exact Manual Replacement payload against the selected source text, replays the exact append-only decision sequence through existing ledger validation, verifies bound session-authority consistency with source revision and analysis snapshot, and compares analysis identity, ordered cases, ledger events, effective status, progress, summary, current projection, reviewed-output result, and export-bundle equality. When a project scope and reuse-governance history are present, same-process replay also compares reuse-governance ledger fold, reusable snapshot identity, reuse-enabled analysis binding, and export v3 bundle equality. This is deterministic same-process in-memory re-execution only. It is not persistence, serialization, restart recovery, cross-process replay, transport, or hydration.

**Gate 3 reuse slice (bounded implementation, owner-accepted at `225e1b3fc6cc0403d202d1d72fe7325a97dc3096`; MD-018 semantics accepted at `a55f76b842b26e882f5210f6cc2ca601b1cf282c`):** Accepted MD-018 semantics are implemented through a separate reuse-governance ledger, derived promotion candidates, explicit accept/reject/revoke/supersede commands, deterministic reusable snapshots, a typed exact-input projection with fail-closed conflict refusal, an explicit reuse-enabled analysis path, and application export v3 renderers. Session-local reuse-governance persistence is owned by MD-020. Cross-material Project Memory, v2 session binding, derived proposals, and thin decision targets are owned by MD-021. Export v3 is not a public pack format and is not hydration input. Frozen application export v2 identities and golden outputs remain unchanged when no project scope is configured. This acceptance does not establish v0.3 or authorize deterministic cross-material reuse demonstration.
