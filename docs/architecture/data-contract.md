...
**Gate 3 reuse slice (bounded in-memory implementation, owner-accepted at `225e1b3fc6cc0403d202d1d72fe7325a97dc3096`; MD-018 semantics accepted at `a55f76b842b26e882f5210f6cc2ca601b1cf282c`):** Accepted MD-018 semantics are implemented in memory only through a separate reuse-governance ledger, derived promotion candidates, explicit accept/reject/revoke/supersede commands, deterministic reusable snapshots, a typed exact-input projection with fail-closed conflict refusal, an explicit reuse-enabled analysis path, and application export v3 renderers. Reuse authority is not persisted. Export v3 is not a public pack format and is not hydration input. Frozen application export v2 identities and golden outputs remain unchanged when no project scope is configured. This acceptance does not establish v0.3, authorize product-session persistence integration, or authorize deterministic cross-material reuse demonstration.
