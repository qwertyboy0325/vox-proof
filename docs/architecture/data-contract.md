...
Matching proceeds in ordered phases: exact-anchor NFC-equal, exact-anchor correction-different, non-exact overlap (overlap alone never yields accepted overlap), then stable one-to-one primary assignment using lowest `DetectorProposalId` for duplicate competing proposals. One detector positively assigned to multiple references remains ambiguous unless frozen overlap adjudication uniquely selects one edge.
