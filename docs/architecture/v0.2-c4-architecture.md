...
If accepted, proposed `governance/decisions/MD-014-proposed-session-durability-recovery-and-retention-requirements.md` would own durability, lifecycle, recovery, locking, and retention requirements for this flow. Storage mechanism and format remain TBD.
...
| Session store / serialization | Versioned serialized state, restore metadata | Independent ledger semantics or materialization rules | Proposed v0.2 | No implementation yet; durability/recovery/retention requirements owned by proposed MD-014; pre-selection evidence protocol owned by accepted MD-015; storage mechanism TBD |
...
| 4 | Session persistence format & compatibility policy | Touches MD-governed serialization boundary | JSON snapshot; SQLite; directory bundle; no persistence in v0.2 | Migration rules, partial-save semantics, corruption recovery; durability/recovery/retention requirements proposed in MD-014; pre-selection evidence protocol accepted in MD-015; mechanism still TBD | Ezra (Material Decision) |
...
| Session durability, recovery, retention (proposed) | MD-014 |
