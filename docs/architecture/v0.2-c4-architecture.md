...
| Session store / serialization | Versioned serialized state, restore metadata | Independent ledger semantics or materialization rules | Proposed v0.2 | No implementation yet; durability/recovery/retention requirements owned by proposed MD-014; storage mechanism TBD |
...
| 4 | Session persistence format & compatibility policy | Touches MD-governed serialization boundary | JSON snapshot; SQLite; directory bundle; no persistence in v0.2 | Migration rules, partial-save semantics, corruption recovery; durability/recovery/retention requirements proposed in MD-014; pre-selection evidence protocol proposed in MD-015; mechanism still TBD | Ezra (Material Decision) |
...
| Persistence mechanism evidence protocol (proposed) | MD-015 |
