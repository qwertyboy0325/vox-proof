...
Owns: Proposed v0.2 C4 architecture views, trust-boundary analysis, accepted MD-016 native-desktop foundation boundaries, and remaining open architectural decisions pending Ezra approval.
...
Last reviewed against code/evidence: application-service authority and export behavior are implemented; v0.1 established by MD-008; native desktop framework and in-process integration accepted by MD-016; the bounded in-memory Gate 1 desktop implementation is owner-accepted at `0104ea2519cfd18b3ec639e5ec1566b734a6655e`.
...
This document does not by itself establish v0.2 scope or approve a new architecture. Established v0.1 semantics and accepted Material Decisions, including MD-016, remain authoritative. Any additional durable v0.2 architecture choice requires Ezra approval through the Material Decision process (`governance/material-decisions.md`).
...
2. a **proposed v0.2** product architecture for a conditional thin desktop review surface using the foundation accepted in MD-016;
...
| **Proposed v0.2** | Planning direction only unless a specific boundary is accepted by a cited Material Decision |
...
| Shared correctness semantics | CLI and future product surfaces must reuse the same canonical core correctness and materialization semantics. MD-016 selects direct in-process desktop integration with `ApplicationReviewSession`. | MD-003; MD-016; `architecture/overview.md` |
...
The broader target shape remains proposed. MD-016 accepts eframe/egui `0.35.0`, a single native Rust process, and direct in-process integration with `ApplicationReviewSession`. Responsibilities inside that process are components of the Desktop Application, not separate containers. Persistence technology remains TBD.
...
flowchart TB
  ...
  subgraph v02["VoxProof proposed v0.2"]
    ...
    desktop["Container: VoxProof Desktop Application<br/>eframe/egui 0.35.0<br/>single native Rust process"]
...
| VoxProof Desktop Application | Single native Rust process/executable containing eframe/egui presentation, direct application-service integration, and linked core semantics | Bounded in-memory Gate 1 implementation owner-accepted at `0104ea2519cfd18b3ec639e5ec1566b734a6655e` |
...
In-process responsibilities inside the Desktop Application (not Level 2 containers): eframe/egui Review UI, direct `ApplicationReviewSession` integration, Filesystem Adapter, optional future Session Serialization and Media Playback Adapters, and reused linked core components.

Do **not** read this diagram as approval of SQLite, macOS-only release, a specific session serialization format, packaging, signing, IME readiness, or accessibility conformance. MD-016 rejects Tauri/webview and cross-process integration for this foundation.

## Level 3 — Desktop Application Components

Highest-risk in-process boundary for v0.2 planning: components of the proposed **VoxProof Desktop Application**. MD-016 fixes the shell as eframe/egui and the application integration as direct calls to `ApplicationReviewSession`. Existing core semantics remain linked/in-process components reused by that executable. The Structurizr component view targets this container only.

```mermaid
flowchart TB
  subgraph desktop["Desktop Application proposed"]
    subgraph proposed["Proposed application layer"]
      reviewUI["eframe/egui Review UI<br/>non-authoritative"]
      appSession["ApplicationReviewSession<br/>canonical application state"]
  ...
  reviewUI --> appSession
  reviewUI --> fsAdapter
  reviewUI --> mediaAdapter
  fsAdapter --> srtParser
  appSession --> transcript
  appSession --> detectors
  appSession --> ledger
  appSession --> materializer
  appSession --> validation
  sessionSer -.-> appSession
  materializer --> ledger
  materializer --> transcript
```

`ApplicationReviewSession` deliberately has no filesystem, media, network, GUI, or persistence authority. Filesystem import/export remains outside the session boundary. The dashed serialization relationship is future-only and does not imply that hydration or persistence is accepted or implemented.

### Component classification summary

| Component area | Classification | Ownership |
| --- | --- | --- |
| eframe/egui Review UI | Bounded in-memory Gate 1 implementation owner-accepted at `0104ea2519cfd18b3ec639e5ec1566b734a6655e` | Presentation and transient GUI state only; not authoritative |
| `ApplicationReviewSession` | Application-service boundary implemented; desktop integration accepted by MD-016 | Canonical in-memory application state; delegates core semantics and owns no I/O |
| Session Serialization Adapter | Proposed v0.2 | Infrastructure; not core domain |
| Filesystem Adapter | Bounded Gate 1 implementation owner-accepted at `0104ea2519cfd18b3ec639e5ec1566b734a6655e` | SRT and session-term input plus authority-complete three-file export; no persistence, crash-safe transaction, or overwrite claim |
| Media Playback Adapter | Proposed v0.2; not implemented | Infrastructure; media has no correction authority |
...
For the established CLI executable, the same linked modules exist today inside `vox-proof` / `src/main.rs` without a separate application orchestrator. That CLI internal component tree is established runtime reality. The broader Desktop Application component view remains a proposed v0.2 planning surface, while the table above distinguishes the bounded branch implementations of the Gate 1 Review UI and Filesystem Adapter from future components.
...
```text
Reviewer → eframe/egui Desktop Application → ApplicationReviewSession
...
Accepted `governance/decisions/MD-014-proposed-session-durability-recovery-and-retention-requirements.md` owns durability, lifecycle, recovery, locking, and retention requirements for this flow. Storage mechanism and format remain TBD.
...
| --- | --- | --- | --- | --- |
| Product UI (CLI prompts today; eframe/egui Review UI accepted) | Presentation, navigation, human intent capture, export triggers | Canonical decisions, reviewed SRT text, detector configuration truth | CLI established; bounded in-memory Gate 1 GUI implementation owner-accepted at `0104ea2519cfd18b3ec639e5ec1566b734a6655e` | `architecture/overview.md`; MD-002; MD-016 |
| Application layer (`ApplicationReviewSession`) | Canonical in-memory application state, use-case sequencing, decision submission, projection and export access | Independent correction semantics; alternate materialization; durability claims | Application service implemented; direct desktop integration accepted | `src/application_service.rs`; MD-016 |
...
| Session store / serialization | Versioned serialized state, restore metadata | Independent ledger semantics or materialization rules | Proposed v0.2 | No implementation yet; durability/recovery/retention requirements owned by accepted MD-014; pre-selection evidence protocol owned by accepted MD-015; storage mechanism TBD |
...
| Filesystem adapter | Path read/write, artifact naming | Domain validation bypass | CLI I/O established; bounded Gate 1 desktop adapter owner-accepted at `0104ea2519cfd18b3ec639e5ec1566b734a6655e` | `main.rs`; `apps/desktop/src/controller.rs`; `apps/desktop/src/export.rs` |
...
These are **candidate v0.2 constraints** for planning discussion except where a row cites an accepted Material Decision. They are not established product facts beyond that cited boundary.
...
| Native eframe/egui foundation | MD-016 selects eframe/egui `0.35.0`, one Rust process, direct `ApplicationReviewSession` integration, and non-authoritative GUI state |
...
| --- | --- | --- | --- | --- | --- |
| 1 | Desktop shell / framework | Defines process model, UI technology, and core interop | **Resolved by MD-016:** eframe/egui `0.35.0`, single native Rust process | IME composition and accessibility inspection remain open validation risks | Accepted by Ezra in MD-016 |
...
| 4 | Session persistence format & compatibility policy | Touches MD-governed serialization boundary | JSON snapshot; SQLite; directory bundle; no persistence in v0.2 | Migration rules, partial-save semantics, corruption recovery; durability/recovery/retention requirements accepted in MD-014; pre-selection evidence protocol accepted in MD-015; mechanism still TBD | Ezra (Material Decision) |
| 5 | Application-service / API boundary | Determines whether UI calls Rust via FFI, IPC, or in-process crate | **Resolved by MD-016:** direct in-process `ApplicationReviewSession` integration | Persistence and restart recovery remain separate | Accepted by Ezra in MD-016 |
...
| 7 | Manual correction as distinct decision form | **Resolved by MD-017:** exact validated single-line payload on an existing detector-raised ReviewCase, recorded as `DecisionRecorded`, sharing canonical materialization and overlap rules | Implemented on the current branch for remote milestone review | HumanRaised cases, deletion, multiline editing, withdrawal, persistence, and Correction Memory remain separate | Accepted by Ezra in MD-017 |
...
| 10 | New Material Decision required? | Governance gate before implementation | **Partially resolved:** MD-016 accepts the native desktop foundation; other durable boundaries remain split decisions | Which remaining rows block later GUI capabilities | Ezra |

Rows 1 and 5 are resolved by MD-016. Row 7 is resolved only for the bounded existing detector-raised ReviewCase slice accepted by MD-017. Other rows remain proposed or open and are not silently decided by either decision.
...
| Session durability, recovery, retention (accepted) | MD-014 |
| Persistence mechanism evidence protocol (accepted) | MD-015 |
| Native desktop framework and in-process integration (accepted) | MD-016 |
...
- [ ] Only MD-016 technology is named as decided; persistence and other technologies remain TBD
...
- [x] Native desktop framework and integration boundary accepted in MD-016
- [x] Bounded owner-authorized implementation work package recorded before desktop implementation starts
