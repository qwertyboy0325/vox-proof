...
Last reviewed against code/evidence: application-service authority and export behavior are implemented; v0.1 established by MD-008; native desktop framework and in-process integration accepted by MD-016; a bounded in-memory desktop implementation is present on the current branch for remote owner review.
...
| VoxProof Desktop Application | Single native Rust process/executable containing eframe/egui presentation, direct application-service integration, and linked core semantics | Bounded branch implementation present; remote owner review pending |
...
| --- | --- | --- |
| eframe/egui Review UI | Bounded branch implementation present; remote owner review pending | Presentation and transient GUI state only; not authoritative |
...
| Filesystem Adapter | Bounded branch implementation present; remote owner review pending | SRT and session-term input plus authority-complete three-file export; no persistence, crash-safe transaction, or overwrite claim |
...
| --- | --- | --- | --- | --- |
| Product UI (CLI prompts today; eframe/egui Review UI accepted) | Presentation, navigation, human intent capture, export triggers | Canonical decisions, reviewed SRT text, detector configuration truth | CLI established; bounded GUI branch implementation present; remote owner review pending | `architecture/overview.md`; MD-002; MD-016 |
...
| Filesystem adapter | Path read/write, artifact naming | Domain validation bypass | CLI I/O established; bounded desktop adapter branch implementation present; remote owner review pending | `main.rs`; `apps/desktop/src/controller.rs`; `apps/desktop/src/export.rs` |
