...
current wedge; v0.2 is in progress; Gate 3 reusable influence is accepted and
implemented in memory only; product-session persistence integration, built-in
ASR, media playback, durable Correction Memory, and deterministic
cross-material reuse demonstration remain unimplemented.
...
concept does not select a persistence mechanism or assert durable
...
and its in-memory implementation does not establish durable Correction Memory,
cross-material proof, or v0.3.
