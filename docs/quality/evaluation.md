...
The real-transcript runner request contract validates only protocol readiness for a future detector-execution implementation. It requires confirmed authorization, six immutable blind-reference lifecycle envelopes, a sealed blind-eligible complete human-reference stack, a frozen detector-analysis identity, and the exact nine-role artifact inventory including `InputAuthorization`. The request-level inventory is directly bound to the common six-envelope inventory; synchronized non-canonical tampering is rejected and duplicate roles return a structured duplicate-role error. Lifecycle transitions consume the validated common envelope calibration mode (`BlindReference` only). Future overlap adjudication must use owner or authorized-domain human authority; `SyntheticFixtureAdjudicator` is forbidden for real overlap resolution.

Readiness means `ReadyForDetectorExecution` only. No detector execution, human adjudication collection, metric derivation, packet construction, filesystem write, CLI, or real-run execution occurs in the readiness contract slice. Existing packet v1 and the file adapter remain synthetic-only; real packet transport remains deferred. Contract tests use artificial deterministic fixtures and do not establish real-material evidence.

## v0.2 Real-Transcript Evaluation Execution Boundary

`real_transcript_evaluation_execution` consumes an accepted real-run request plus a caller-supplied frozen detector snapshot. It revalidates the request before any join or metric derivation, enforces an empty frozen `DetectorExecution` adjudication set, and returns a typed human-adjudication requirement when overlap remains unresolved. Supplied assisted-review adjudication is validated and applied; it is not collected by the runner. Real overlap resolution accepts only owner or authorized-domain human roles.

Completed results derive join, contribution, and aggregate artifacts, serialize an exact nine-role in-memory artifact set including `InputAuthorization`, build a complete `ArtifactBundle`, rederive the chain against the final bundle with exact typed equality, and replay all decoded artifacts at `Finalized`. Public completed-result verification re-enforces human adjudicator authority, revalidates frozen detector snapshot authority (including exact analysis-identity binding to the validated plan), binds `completion_stage` to the adjudication/join path, validates the exact deterministic execution trace, binds bundle descriptor schemas to artifact roles, and rejects hash-synchronized authority or derived-artifact tampering. Bootstrap bundles and stubs are internal only. Payload bytes remain in memory; no packet, filesystem, CLI, persistence, or real-material input was introduced. Artificial real-posture fixtures do not establish actual real-material evidence, legal sufficiency, or detector effectiveness. Derived metric flags and primary-evidence eligibility reflect typed declarations and contract derivation only; no threshold or product pass/fail is computed.

## v0.2 Real-Transcript Detector Snapshot Adapter Boundary

`real_transcript_detector_snapshot_adapter` validates snapshot-materialization readiness for one accepted real-run request, one in-memory transcript, and one branded `CanonicalTermReviewRun`. It revalidates the runner request, binds transcript and analysis identities to the validated plan, derives detector analysis identity from `AnalysisRun`, validates explicit proposal-id inventory and canonical review-case order, and preflights lossless candidate-to-proposal mapping rules including checked integer conversions, phonetic comparison semantics, future semantic-key uniqueness, and transcript-resolved observed surfaces. Production contract code does not execute detectors or call the canonical pipeline. Successful validation means `ReadyForSnapshotMaterialization` only; no `DetectorProposalSnapshot` is created, no real material is consumed, and caller-supplied frozen timestamps are not independently authenticated. Contract tests use artificial fixtures only.
